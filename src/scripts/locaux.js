...
$scope.createVolunteer = function() {
    ...
    $scope.names.unshift(newVolunteer);
    Materialize.toast("New volunteer added", 4000);
...
$scope.readOne = function(id) {
    ...
    const recordMatch = $scope.names.find(x => x.id === id);
    // put the values in form
    $scope.id = recordMatch.id;
    $scope.name = recordMatch.name;
    $scope.email = recordMatch.email;
    $scope.phone = recordMatch.phone;
    $scope.status = recordMatch.status;
    $scope.unix = recordMatch.unix;
    // show modal
...
$scope.updateVolunteer = function() {
    const updateIndex = $scope.names.findIndex(x => x.id == $scope.id);
    $scope.names[updateIndex] = {
        'id': $scope.id,
        'name': $scope.name,
        'email': $scope.email,
        'phone': $scope.phone,
    ...
    Materialize.toast("Volunteer record updated", 4000);
...
$scope.deleteVolunteer = function(id) {
    if (confirm("Are you sure?")) {
        const deleteIndex = $scope.names.findIndex(
    x => x.id == $scope.id
  );
        $scope.names.splice(deleteIndex, 1);
...
// Old PHP Post functions (Unused)
const createVolunteer = function() {
...
const updateVolunteer = function() {
